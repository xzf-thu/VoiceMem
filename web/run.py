...
_RB_HUMANIZE_PROMPT = (
    # 两版教训都写在这儿，免得再退回去：
    # ① 只写"第一人称"不够——要说清是**助手**在讲自己的观察，"我"必须出现。
    #    否则模型只做同义替换："在焦虑时倾向于简短回应" → "他焦虑时爱简短回应"。
    # ② "不许新增事实"会被理解成"不许换说法"，于是变成往原句前面贴个"我注意到"。
    #    得把两件事拆开讲：事实不能加，但**措辞必须重说一遍**。
    "下面每行是一条关于某个人的判断，来自一个长期陪着他的语音助手。\n"
    "把每一条改写成这个助手**自己说出来的观察**——像在跟朋友聊起他，"
    "而不是在念一条档案。\n"
    "\n"
    "要求：\n"
    "· 带上「我」，但**换着说**：我发现 / 我感觉 / 他好像 / 跟他熟了就知道……"
    "十条里别有五条是同一个开头，也别每条都以「我」起头。\n"
    "· **必须换一种说法**，不是在原句前面贴个前缀。原句是概括性的词（"
    "「简短回应」「寻求认同」），要还原成人会怎么形容这件事"
    "（「话就变少了」「想有人接住他」）。\n"
    "· 但**不许新增任何事实**：不补细节、不猜原因、不评价也不安慰。"
    "换的是措辞，不是内容。\n"
    "· 口语，30 字以内，读起来像一句话而不是一个标签。\n"
    "· 原文什么语言就用什么语言；注意别重复用词（不要写出 I feel … feels 这种）。\n"
    "\n"
    ...
    "  输出  A bit of recognition really lifts him\n"
    "  输入  Hates being interrupted\n"
    "  输出  I've noticed he really can't stand being cut off\n"
)
